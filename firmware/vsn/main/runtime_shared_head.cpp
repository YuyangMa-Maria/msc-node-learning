    ...
    float local_threshold)
{
    ...
    if (g_lock == nullptr || !allocate_slots() ||
        !model_exchange::valid_package_contract(initial_package.format, initial_package.size)) {
        return false;
    }
    float golden = 0.0f;
    if (!model_exchange::evaluate_shared_head(
            initial_package.data,
            initial_package.size,
            initial_package.format,
            golden) ||
        std::fabs(golden - initial_package.expected_golden_output) >= 1.0e-4f) {
        return false;
    }
    xSemaphoreTake(g_lock, portMAX_DELAY);
    std::memcpy(g_slots[0].payload, initial_package.data, initial_package.size);
    g_slots[0].payload_bytes = initial_package.size;
    g_slots[0].format = initial_package.format;
    g_slots[0].version = initial_package.model_version;
    g_slots[0].threshold = local_threshold;
    ...
    RuntimeHeadTransition &transition)
{
    ...
    if (g_lock == nullptr || payload == nullptr ||
        !model_exchange::evaluate_shared_head(payload, payload_bytes, format, golden)) {
        return false;
    }
    transition.golden_output = golden;
    transition.golden_error = std::fabs(golden - expected_golden_output);
    if (transition.golden_error >= 1.0e-4f) {
        return false;
    }
    ...
    float candidate_live_logit = 0.0f;
    if (g_has_last_representation &&
        !model_exchange::evaluate_shared_head_with_embedding(
            payload, payload_bytes, format, g_last_representation, candidate_live_logit)) {
        xSemaphoreGive(g_lock);
        return false;
    }
    ...
    transition.live_logit_after = candidate_live_logit;
    transition.after = snapshot(candidate);
    xSemaphoreGive(g_lock);
    transition.elapsed_us = esp_timer_get_time() - started_us;
    return true;
}
...
bool runtime_shared_head_rollback(RuntimeHeadTransition &transition)
{
    ...
    transition.had_live_representation = g_has_last_representation;
    if (g_has_last_representation) {
        if (!evaluate_slot(current, g_last_representation, transition.live_logit_before) ||
...
{
    if (g_lock == nullptr || representation == nullptr || values != 64) {
        return false;
    }
    xSemaphoreTake(g_lock, portMAX_DELAY);
